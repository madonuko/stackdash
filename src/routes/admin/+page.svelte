<script lang="ts">
	...
	let vtError = $state('');
	let vtName = $state('');
	let vtIsa = $state<'x86' | 'arm' | 'risc-v'>('x86');
	let vtCores = $state(1);
	let vtRam = $state(512);
	let vtStorage = $state(10);
	let vtRate = $state('0.00');
	let vtCap = $state('5.00');
	...
	function vtOpenEdit(vt: VmType) {
		...
		vtIsa = vt.isa as any;
		vtCores = vt.cores;
		vtRam = vt.ramCapacity;
		vtStorage = vt.storageAmount;
		vtRate = vt.rate;
		vtCap = vt.cap;
		vtError = '';
		vtDialogOpen = true;
	...
	async function vtSave() {
		...
		try {
			...
			vtError = err instanceof Error ? err.message : 'Failed to save';
		} finally {
			vtSaving = false;
		}
	}
	async function vtRemove(id: string) {
		try {
			...
			vmTypes = vmTypes.filter((v) => v.id !== id);
		} catch (err) {
	...
	function imgOpenEdit(img: BaseImage) {
		...
		imgShortName = img.shortName;
		imgIcon = img.icon ?? '';
		imgColor = img.color;
		imgFilePath = img.filePath;
		imgIsa = img.isa as any;
		imgError = '';
		imgDialogOpen = true;
		if (pveIsos.length === 0) loadPveIsos();
	}
	async function imgSave() {
		...
		try {
			...
			imgError = err instanceof Error ? err.message : 'Failed to save';
		} finally {
			imgSaving = false;
		}
	}
...
<div class="flex flex-1 flex-col overflow-hidden">
	...
	<div class="flex h-10 shrink-0 items-center gap-0 border-b border-gray-800">
		...
		{#if activeTab === 'vmTypes'}
			<div class="px-4">
				<Button size="sm" class="h-7 gap-1.5 text-xs" onclick={vtOpenCreate}
...
<Dialog.Root bind:open={vtDialogOpen}>
	<Dialog.Content class="border-gray-800 bg-gray-900 sm:max-w-md">
		<Dialog.Header>
			...
			<Dialog.Description
				>{vtEditing ? 'Update plan specifications.' : 'Define a new VM plan.'}</Dialog.Description
			>
		</Dialog.Header>
		<div class="flex flex-col gap-4 py-4">
			...
			<div class="flex flex-col gap-2">
				<Label>Name</Label><Input bind:value={vtName} placeholder="STACK-XXS" />
			</div>
			<div class="flex flex-col gap-2">
			...
			<div class="grid grid-cols-3 gap-3">
				<div class="flex flex-col gap-2">
					<Label>Cores</Label><Input type="number" bind:value={vtCores} min="1" />
				</div>
				<div class="flex flex-col gap-2">
					<Label>RAM (MB)</Label><Input type="number" bind:value={vtRam} min="128" step="128" />
				</div>
				<div class="flex flex-col gap-2">
					<Label>Storage (GB)</Label><Input type="number" bind:value={vtStorage} min="1" />
				</div>
			</div>
			<div class="grid grid-cols-2 gap-3">
				<div class="flex flex-col gap-2">
					<Label>Rate ($/hr)</Label><Input bind:value={vtRate} placeholder="0.007" />
				</div>
				<div class="flex flex-col gap-2">
					<Label>Cap ($/mo)</Label><Input bind:value={vtCap} placeholder="5.00" />
				</div>
			</div>
		</div>
		<Dialog.Footer>
...
<Dialog.Root bind:open={imgDialogOpen}>
	<Dialog.Content class="border-gray-800 bg-gray-900 sm:max-w-lg">
		...
		<div class="flex flex-col gap-4 py-4">
			...
			<div class="grid grid-cols-2 gap-3">
				<div class="flex flex-col gap-2">
					<Label>Name</Label><Input bind:value={imgName} placeholder="Fedora Server" />
				</div>
				<div class="flex flex-col gap-2">
					<Label>Version</Label><Input bind:value={imgVersion} placeholder="42" />
				</div>
			</div>
			...
			<div class="flex flex-col gap-2">
				<div class="flex items-center justify-between">
					...
					<Button
						...
						disabled={isoLoading}
					>
						{#if isoLoading}<Loader2 class="h-3 w-3 animate-spin" />{:else}<RefreshCw
								class="h-3 w-3"
							/>{/if}
						Scan Nodes
					</Button>
		...
		<Dialog.Footer>
			...
			<Button
				size="sm"
				onclick={imgSave}
				disabled={imgSaving || !imgName.trim() || !imgFilePath.trim()}
			>
