...
export async function setupVMIPAM() {
  ...
  let vm_interface_create = await netbox(
  ...
  let mac_address_create = await netbox(
  ...
  let ip_address_create = await netbox(
  ...
  let vm_interface_patch_add_mac = await netbox(
    '/api/virtualization/interfaces/',
    "PATCH",
    [{
